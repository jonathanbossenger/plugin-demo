# Copilot Instructions for plugin-demo

## Repository Overview

This is a WordPress plugin that demonstrates WordPress block development using the Gutenberg block editor. The plugin creates a custom block called "Plugin Demo" that displays sample text both in the editor and on the front-end.

**Repository Type:** WordPress Plugin  
**Primary Language:** JavaScript (React/JSX), PHP  
**Frameworks:** WordPress Block Editor (Gutenberg), @wordpress/scripts  
**Target Runtime:** WordPress 6.8+, PHP 8.1+, Node.js 20+  
**Repository Size:** Small (~25 files, excluding dependencies)
...
- **npm:** v10.8.2 (tested and confirmed working)
- **PHP:** 8.3.6 or higher (plugin requires PHP 8.1+)
...
- Creates minified JavaScript, CSS, and asset dependency files in `build/plugin-demo/`
...
**PHP Linting:**
```bash
./vendor/bin/phpcs plugin-demo.php
```
...
- The main plugin file has known PHPCS warnings (tabs vs spaces, line length) - these are acceptable per project style
- Auto-fix many PHP issues with: `./vendor/bin/phpcbf plugin-demo.php`
...
npm run plugin-zip
```
- Creates `plugin-demo.zip` in the repository root (ignored by git)
- Includes only necessary files: `plugin-demo.php`, `build/` directory contents
...
├── plugin-demo.php            # Main plugin file - registers blocks
└── src/
    └── plugin-demo/           # Block source files
...
**`plugin-demo.php`** - Main plugin entry point
- Contains WordPress plugin header with metadata (name, version, requires WordPress 6.8+, requires PHP 8.1+)
...
- Function: `jonathanbossenger_plugin_demo_block_init()` hooked to `init`

**`src/plugin-demo/block.json`** - Block metadata (WordPress Block API v3)
- Defines block name: `jonathanbossenger/plugin-demo`
- Specifies editor/view scripts and styles
- Schema validation via `$schema` property

**`src/plugin-demo/edit.js`** - Editor component
...
**`src/plugin-demo/save.js`** - Save component  
...
5. Verify build output exists in `build/plugin-demo/`

**For PHP changes only:**
1. Format and lint as above (if any JS/CSS was touched)
2. Check PHP: `./vendor/bin/phpcs plugin-demo.php` (warnings acceptable)
...
- **Solution:** Check for errors in console; ensure `src/plugin-demo/` files exist
...
- All source files are in `src/plugin-demo/`, all build outputs go to `build/plugin-demo/`
- This project follows WordPress coding standards, which use TABS for indentation
- There are no tests in this repository
- There are no GitHub Actions workflows or CI/CD pipelines
- The repository has no README.md or CONTRIBUTING.md files
